...
export function WEvent({ dark }: WEventProps) {
  ...
  return (
    <Glass dark={dark} span={12} pad={14} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ flex: 1 }}>
        <div className="badge" style={{ fontSize: 12 }}>WEST LINE WINDER 30K</div>
      ...
      <div style={{ textAlign: 'right' }}>
        <div className="mono" style={{ fontSize: 18, fontWeight: 700, lineHeight: 1 }}>
          {days}<span style={{ fontSize: 10, opacity: 0.5 }}>d</span>
        </div>
        <div className="mono" style={{ fontSize: 9, color: C.teal }}>register now ↗</div>
      </div>
    </Glass>
  )
}
