...
export function W4PM({ dark }: W4PMProps) {
    ...
    return (
      <Glass dark={dark} span={12} pad={16}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <span className="mono" style={{ fontSize: 'var(--fs-11)', color: weatherColor }}>{weatherTag}</span>
        </div>
        <div style={{ fontSize: 'var(--fs-14)', opacity: 0.45, marginTop: 8 }}>No tasks in queue — add some in Lists.</div>
      </Glass>
    )
  }
  ...
  return (
    <Glass dark={dark} span={12} pad={16}>
      ...
      <div className="badge" style={{ fontSize: 'var(--fs-17)', marginTop: 4, lineHeight: 1.15 }}>
        {pick.title.toUpperCase()}
      </div>
      {pick.notes && (
        <div style={{ fontSize: 'var(--fs-13)', opacity: 0.65, marginTop: 4, lineHeight: 1.4 }}>{pick.notes}</div>
      )}
      <div className="mono" style={{ display: 'flex', gap: 6, marginTop: 10, fontSize: 'var(--fs-12)', flexWrap: 'wrap' }}>
        {pick.effort && (
          <span style={{
            background: 'var(--color-teal-20)', color: dark ? C.teal : C.dark,
