  ...
  return (
    <Glass dark={dark} span={5} pad={14}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4 }}>
        ...
        <div>
          <div className="badge" style={{ fontSize: 13, color: result ? color : C.ink40 }}>
            {TIER_LABEL[tier]}
          </div>
          <div className="mono" style={{ fontSize: 9.5, opacity: 0.6, marginTop: 2 }}>
            {rhr ? `RHR ${rhr}` : 'no RHR'} · {sleep != null ? `sleep ${Math.round(sleep * 10) / 10}h` : 'no sleep'}
          </div>
          <div className="mono" style={{ fontSize: 9.5, opacity: 0.6 }}>
            conf · {conf}
          </div>
        </div>
      </div>
