        ...
        {loading ? (
          ...
          items.map(item => (
            <div key={item.id} style={{
              ...
              <div style={{ fontSize: 'var(--fs-15)', lineHeight: 1.4, color: C.dark }}>{item.content}</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, alignItems: 'center' }}>
                <span className="mono" style={{ fontSize: 'var(--fs-12)', color: C.ink60 }}>{timeAgo(item.captured_at)}</span>
                <div style={{ display: 'flex', gap: 6 }}>
                  <button
                    onClick={() => handleRoute(item.id)}
                    style={{
                      fontSize: 'var(--fs-12)', padding: '4px 9px', borderRadius: 999,
