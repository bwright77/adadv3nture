...
function DirectionChip({ direction, isGood }: { direction: 'up' | 'down' | 'flat' | null; isGood: boolean | null }) {
  ...
  return (
    <span style={{
      ...
      fontSize: 'var(--fs-11)', fontWeight: 700, lineHeight: '16px', textAlign: 'center',
      flexShrink: 0,
    }}>
      {neutral ? '→' : isGood ? '✓' : '↑'}
    </span>
    ...
    <div style={{ position: 'relative', zIndex: 10 }}>
      ...
      <div style={{ padding: '0 16px 100px' }}>
        ...
        <div style={{
          ...
          {loading ? (
            <div style={{ padding: 20, fontSize: 'var(--fs-14)', opacity: 0.4 }}>Loading trends…</div>
          ) : rows.length === 0 ? (
            <div style={{ padding: 20, fontSize: 'var(--fs-14)', opacity: 0.4 }}>No data yet.</div>
          ) : rows.map((row, i) => (
            <div key={i} style={{
              padding: '13px 16px',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div className="mono" style={{
                  fontSize: 'var(--fs-17)', fontWeight: 700,
                  color: row.isHero ? C.rust : C.dark,
                }}>
                  {row.value}
                </div>
          ...
          {loading ? (
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div className="badge" style={{ fontSize: 'var(--fs-15)' }}>{r.label}</div>
                  <div className="mono" style={{ fontSize: 'var(--fs-12)', color: C.ink60, marginTop: 3 }}>
                    {r.daysUntil} days out · {Math.floor(r.daysUntil / 7)} weeks
                  </div>
                  {r.longestRunMiles != null && (
                    <div className="mono" style={{ fontSize: 'var(--fs-12)', color: C.ink60, marginTop: 1 }}>
                      longest run: {r.longestRunMiles.toFixed(1)} mi
                      {r.weeklyMilesAvg != null && ` · avg ${r.weeklyMilesAvg.toFixed(1)} mi/wk`}
                    </div>
                  )}
                  <div className="mono" style={{ fontSize: 'var(--fs-12)', color: C.rust, marginTop: 3 }}>
                    ↳ {r.nextMilestone}
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  {['Run base', 'Cycling', 'Trail focus', 'Race'].map((phase, i) => (
                    <div key={i} className="mono" style={{
                      fontSize: 'var(--fs-10)', opacity: i === 0 ? 1 : 0.4,
                ...
                <div className="mono" style={{ fontSize: 'var(--fs-10)', marginTop: 3, opacity: 0.5 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px', marginTop: 8 }}>
            {[
              ...
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <span style={{ fontSize: 'var(--fs-12)', opacity: 0.55 }}>{label}</span>
                <span className="mono" style={{ fontSize: 'var(--fs-12)', fontWeight: 700 }}>{val}</span>
              </div>
            ))}
          </div>
        </div>
