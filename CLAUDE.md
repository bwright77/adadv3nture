...
| Family Hikes tracker (formerly "50 Hikes"; migration 042 renamed `hikes_50`→`family_hikes`, `use50Hikes`→`useFamilyHikes`, `W50Hikes`/`Hikes50View`→`WFamilyHikes`/`FamilyHikesView`) — an **open, aspirational collection** of hikes done together (no book-50 goal, no completion ring — delight not pace, never nagged). The original 50 + family-added hikes (＋ Add a hike, `is_custom` flag, nullable `book_number`) all live as one flat list; book/custom distinction is no longer surfaced in the UI. Count of hikes done + **geo-aware suggestion** (`suggestHike` filters undone hikes by `haversineMi` from current GPS ≤90mi, closest-first, in-season preferred; unplaced hikes always qualify; nothing reachable → null), log, expanded list. The book is **statewide Colorado**; `trailhead_lat/lng` backfilled by geocoding each hub (`geocodePlace` in `openweather.ts`, **Colorado-constrained** — OWM `,CO,US` bias is unreliable for homonym towns). ＋ Add a hike takes an AllTrails URL (stored/tappable; AllTrails itself is unscrapable — 403) + an Area that's geocoded to coords on save | ✓ |
...
18. **Summer Mode (Jun 2–Aug 26) re-weights, never hides** — auto-active on date; "normal day ›" in the hero overrides off. Adventure on top, structure underneath. Watcher narrows what may **interrupt** to Fire + WA(5×/wk) + Training only; **pilot lights stay visible** (Family/Home tracked-but-silent ≠ hidden — see memory). Adventure is delight-only, never nagged (got-out dot + weekly real-adventure star; missed week = unlit cell, never a reset, à la drinks-ratio). Week-type: **Camp is schedule-driven** — `CAMP_WEEKS` in `src/hooks/useSummerMode.ts` (Jun 8–12, Jul 6–10 2026) mirrored in the briefing fn; during a camp week it's always 'camp', otherwise Solo (or a Weekend choice). The hero toggle persists `users.summer_week_type` for Solo/Weekend; the briefing reads schedule + stored server-side (a stale stored 'camp' never bleeds into a non-camp week).
